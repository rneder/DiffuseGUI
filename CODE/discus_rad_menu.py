...
def discus_rad_menu(parent, nwave, radiation, wvle, ener, ElementNumber, adp, ano, pos_row, pos_col):
        ...
        parent.rad = tk.Listbox(parent, height=3, width=10,selectbackground='#FFFFFF',
        ...
        parent.R0 = tk.Radiobutton(parent,text='Element    ',variable=parent.rad_type,value=2,
                  activeforeground='#F00',foreground='#00F', justify='left',
                  command=lambda: convert_setup(parent, 3))
        parent.R1 = tk.Radiobutton(parent,text='Wave length',variable=parent.rad_type,value=0,
                  activeforeground='#F00',foreground='#00F', justify='left',
                  command=lambda: convert_setup(parent, 1))
        parent.R2 = tk.Radiobutton(parent,text='Energy',variable=parent.rad_type,value=1,
                  activeforeground='#F00',foreground='#00F', justify='left',
                  command=lambda: convert_setup(parent,-1))
        #
        parent.yScroll = tk.Scrollbar(parent, orient=tk.VERTICAL)
        parent.ele = tk.Listbox(parent, height=1, width= 5,selectbackground='#FFFFFF',
                   selectforeground='#0000FF',selectmode=tk.SINGLE,
        ...
        parent.entry_wvle = ttk.Entry(parent,textvariable=parent.wvle, width=10, justify='right')
        parent.entry_ener = ttk.Entry(parent,textvariable=parent.ener, width=10, justify='right')
        #
